...
const RepaymentForm = () => {
  ...
  console.log(CaptchaCodeModal);

  console.log(router);

  const [captchaCodeModalVisible, setCaptchaCodeModalVisible] = useState(false);
  ...
  const handleClick = () => {
    setCaptchaCodeModalVisible(true);
    return;
    const submitData = {
  ...
  return (
    <>
      <CaptchaCodeModal
        title="支付验证码"
        visible={captchaCodeModalVisible}
        phoneNumber={"139****1239"}
        onNext={(code) => setCaptchaCodeModalVisible(false)}
      />
      <View>
        <Cell.Group>
          <Cell
        ...
        <Cell.Group>
          ...
          <Cell
            ...
            onClick={() =>
              setAdvanceLoanAmountVisible(!advanceLoanAmountVisible)
            }
          />
          <Picker
        ...
        <View>
          <Text>温馨提示：</Text>
          <Text>
            每天18点到24点之间提前还款可能会失败，建议在其他时段 操作。
          </Text>
        </View>
